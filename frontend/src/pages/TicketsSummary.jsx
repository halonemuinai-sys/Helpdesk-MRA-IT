...
export default function TicketsSummary({ user, token }) {
  ...
  // Reset selection on filter or list changes
  useEffect(() => {
  ...
  const handleBulkClose = async (idsToClose) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Gagal',
        text: err.message || 'Terjadi kesalahan saat memproses penutupan tiket.',
        confirmButtonColor: '#10b981'
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6 animate-fade-in relative min-h-screen">
      ...
      {user.role === 'ADMIN' && (resolvedTickets.length > 0 || selectedTicketIds.length > 0) && (
        <div className="bg-emerald-50/40 dark:bg-emerald-950/10 border border-emerald-200/50 dark:border-emerald-900/30 p-3.5 rounded-2xl flex flex-wrap items-center justify-between gap-3 animate-slide-up shadow-sm mb-4">
          ...
          <div className="flex items-center gap-2">
            {selectedTicketIds.length > 0 ? (
              ...
              <button
                type="button"
                onClick={() => handleBulkClose(resolvedTickets.map(t => t.id))}
