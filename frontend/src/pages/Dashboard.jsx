...
export default function Dashboard({ user, token, darkMode }) {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // 3b. Fetch Asset lease / Subscription expiry within 30 days (AGENT/ADMIN only)
      if (user.role !== 'USER') {
        try {
          const expiryRes = await fetch(`${API_URL}/reports/expiring-soon`, { headers });
          if (expiryRes.ok) {
            setExpiringSoon(await expiryRes.json());
          }
        } catch (e) {
          console.error("Failed to fetch expiring-soon report:", e);
        }
      }
  ...
  // Notification count: urgent + overdue tickets + expiring assets/subscriptions
